  ...
  try {
    ...
    switch (propertyData.descriptionStyle) {
      ...
      case "فاخر":
        ...
        break;
      case "عائلي":
        stylePrompt = "اكتب وصفاً موجهاً للعائلات يبرز ملاءمة العقار للحياة الأسرية: الخصوصية، عدد الغرف، المجالس، المطبخ، الأمان، قرب المدارس والحدائق والمساجد، وراحة الأطفال وكبار السن";
        break;
      case "عزاب":
        stylePrompt = "اكتب وصفاً موجهاً للعزاب والشباب يبرز سهولة السكن المنفرد: الموقع الحيوي، قرب العمل والمطاعم والخدمات، البساطة والعملية، التأثيث الجاهز إن وجد، والمساحات المناسبة للسكن الفردي";
        break;
      default:
...
${propertyData.warranties && propertyData.warranties.length > 0 ? `- الضمانات: ${propertyData.warranties.map(w => `${w.type} (${w.duration})`).join('، ')}` : ''}
    ...
    // تحديد إذا كان إيجار وله دفعات
    const isRent = /إيجار|ايجار|rent/i.test(propertyData.purpose || '');
    const pp = propertyData.paymentPrices || {};
    const rentPaymentsLines: string[] = [];
    if (isRent) {
      if (pp.onePayment) rentPaymentsLines.push(`• دفعة واحدة: ${pp.onePayment} ريال`);
      if (pp.twoPayments) rentPaymentsLines.push(`• دفعتان: ${pp.twoPayments} ريال لكل دفعة`);
      if (pp.fourPayments) rentPaymentsLines.push(`• أربع دفعات: ${pp.fourPayments} ريال لكل دفعة`);
      if (pp.monthly) rentPaymentsLines.push(`• شهري: ${pp.monthly} ريال شهرياً`);
    }
    const hasRentPayments = rentPaymentsLines.length > 0;

    const priceBlock = propertyData.price
      ? `**السعر**: ${propertyData.price} ريال${isRent ? ' (إيجار)' : ''}`
      : '';
    const rentPaymentsBlock = hasRentPayments
      ? `**خيارات الدفع للإيجار** (يجب ذكر عدد الدفعات وسعر كل دفعة على حدة):\n${rentPaymentsLines.join('\n')}`
      : '';
...
${priceBlock ? `6. ثم اكتب سطر **السعر** بشكل واضح ومنفصل قبل أي معلومات أخرى عن الترخيص أو التواصل: ${priceBlock}` : ''}

${rentPaymentsBlock ? `7. ثم اكتب قسم **خيارات الدفع** بعد السعر مباشرة وقبل الترخيص الإعلاني، مع ذكر عدد الدفعات وسعر كل دفعة على حدة بالضبط كما يلي:\n${rentPaymentsBlock}` : ''}

${propertyData.adLicense || propertyData.brokerPhone ? `8. أخيراً **معلومات التواصل** (يجب أن تأتي بعد السعر${hasRentPayments ? ' وخيارات الدفع' : ''}): ${propertyData.adLicense ? `ترخيص إعلاني: ${propertyData.adLicense}` : ''} ${propertyData.brokerPhone ? `| للتواصل: ${propertyData.brokerPhone}` : ''}` : ''}
...
- ترتيب إلزامي في نهاية الوصف: السعر أولاً، ثم خيارات الدفع (إن وجدت للإيجار)، ثم الترخيص الإعلاني ومعلومات التواصل
- يجب أن يُذكر السعر دائماً قبل الترخيص الإعلاني في جميع الأنماط
${isRent ? '- بما أن الغرض إيجار: يجب ذكر عدد الدفعات وسعر كل دفعة على حدة كما هو مزود في البيانات، بدون دمجها في سطر واحد' : ''}
