...
import { ArrowRight, Loader2, Send, Upload, Home, MapPin, User, Phone, CreditCard, FileText, Building, X, Image as ImageIcon, Video, Star } from 'lucide-react';
      ...
      <PublicFormLayout broker={brokerInfo} title="إرسال عرض عقاري">
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Textarea value={formData.description} onChange={(e) => updateField('description', e.target.value)} placeholder="أدخل وصف تفصيلي للعقار" rows={4} />
          </div>
          ...
          <Button onClick={handleSubmit} disabled={isSubmitting} className="w-full bg-[#01411C] hover:bg-[#065f41] text-white py-6 text-lg">
            {isSubmitting ? <><Loader2 className="w-5 h-5 ml-2 animate-spin" />جاري الإرسال...</> : <><Send className="w-5 h-5 ml-2" />إرسال العرض</>}
          </Button>
        </div>
      </PublicFormLayout>
    </>
