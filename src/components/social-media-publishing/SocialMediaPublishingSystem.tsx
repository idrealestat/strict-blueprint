 ...
 * 3. تيك توك (تكامل TikTok Login Kit + Content Posting)
 * 4. الحملات الإعلانية (5 منصات مدفوعة)
 * 5. الحملات المباشرة (واتساب + تيليجرام)
 * 6. التحليلات (للعرض فقط)
 */

import { useState, useEffect } from 'react';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs';
import { Button } from '@/components/ui/button';
import { ArrowRight, Link2, Video, Megaphone, MessageCircle, BarChart3, Music } from 'lucide-react';
...
import TikTokIntegrationPanel from './TikTokIntegrationPanel';
...
export default function SocialMediaPublishingSystem({ onClose }: SocialMediaPublishingSystemProps) {
  const [activeTab, setActiveTab] = useState<'connections' | 'content' | 'tiktok' | 'campaigns' | 'messaging' | 'analytics'>('connections');
  ...
  return (
    <div
      className="fixed inset-0 z-50 h-[100dvh] w-[100dvw] max-w-[100dvw] flex flex-col bg-background overflow-hidden overflow-x-clip [contain:layout_paint]"
      dir="rtl"
    >
      {/* Header */}
      <div className="p-3 border-b flex items-center gap-2 shrink-0 bg-gradient-to-r from-primary to-primary/80">
        <Button variant="ghost" size="icon" onClick={onClose} className="shrink-0 text-primary-foreground hover:bg-primary-foreground/20">
          <ArrowRight className="w-5 h-5" />
        </Button>
        <div className="min-w-0">
          <h1 className="text-lg font-bold text-primary-foreground truncate">النشر على التواصل الاجتماعي</h1>
          <p className="text-xs text-primary-foreground/70 truncate">انشر محتواك على منصات التواصل</p>
        </div>
      </div>
      ...
      >
        <div className="shrink-0 mt-3 px-2 overflow-x-auto">
          <TabsList className="inline-flex w-auto min-w-full gap-1">
            ...
            <TabsTrigger value="tiktok" className="gap-1 text-xs px-2 whitespace-nowrap bg-gradient-to-r from-[#25F4EE]/10 to-[#FE2C55]/10 data-[state=active]:from-[#25F4EE]/20 data-[state=active]:to-[#FE2C55]/20">
        ...
        <div className="flex-1 min-h-0 overflow-hidden overflow-x-clip w-full max-w-full">
          ...
          <TabsContent value="tiktok" className="h-full m-0 overflow-hidden overflow-x-clip w-full max-w-full">
            <TikTokIntegrationPanel />
          </TabsContent>
