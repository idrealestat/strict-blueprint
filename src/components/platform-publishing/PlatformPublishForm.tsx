...
import { Building, MapPin, Globe, Link, Sparkles, Home, Settings, Shield, Hash, Wand2, Send, Loader2, CheckCircle, AlertCircle, Star, Zap, Copy, RefreshCw, User, Phone, Mail, FileText, CreditCard, Navigation, Plus, DollarSign, Calendar, Clock, Trash2, ExternalLink, Info, Sparkles as SparklesIcon, Check, X, Image, Video, Link2 } from 'lucide-react';
...
import { ExternalPlatform, PlatformPublishedOffer, PlatformPublishResult, AVAILABLE_PLATFORMS } from './types';
...
export default function PlatformPublishForm({
  ...
  useEffect(() => {
    ...
    if (sessionDraft) {
      try {
        ...
        toast.success('✅ تم استرداد البيانات', {
          description: 'تم استعادة بياناتك تلقائياً'
        });
        return;
      } catch {}
    }
    ...
    if (savedDraft) {
      try {
        ...
        const hasData = parsed.ownerName || parsed.ownerPhone || parsed.propertyType || parsed.locationDetails?.city || parsed.media && parsed.media.length > 0;
        if (hasData) {
  ...
  const fetchAddressFromCoordinates = async (lat: number, lng: number) => {
    ...
    try {
      ...
      const response = await fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}&zoom=19&addressdetails=1&namedetails=1&extratags=1&accept-language=ar`);
      ...
      if (data?.address) {
        ...
        setPropertyData(prev => ({
          ...
          ownerNationalAddress: [buildingNum, streetName, district, detectedCity, postalCode].filter(Boolean).join('، ')
        }));
        ...
        if (missingFields.length > 0) {
          ...
          toast.warning(`⚠️ ${missingFields.join(' و ')} غير متوفر في قاعدة البيانات${placeHint} - أدخله يدوياً`, {
            duration: 5000
          });
        } else {
  ...
  const toggleFeature = (f: string) => setPropertyData(prev => ({
    ...prev,
  ...
  const toggleHashtag = (h: string) => setPropertyData(prev => ({
    ...prev,
    hashtags: prev.hashtags.includes(h) ? prev.hashtags.filter(x => x !== h) : [...prev.hashtags, h]
  }));
  const addCustomHashtag = () => {
    if (newCustomHashtag.trim()) {
      setPropertyData(prev => ({
        ...prev,
        customHashtags: [...prev.customHashtags, newCustomHashtag.startsWith('#') ? newCustomHashtag : `#${newCustomHashtag}`]
      }));
      setNewCustomHashtag('');
    }
  };
  const addWarranty = () => {
    const t = newWarrantyType === 'أخرى' ? customWarrantyType : newWarrantyType;
    if (t && newWarrantyDuration) {
      setPropertyData(prev => ({
        ...prev,
        warranties: [...prev.warranties, {
          type: t,
  ...
  const removeWarranty = (i: number) => setPropertyData(prev => ({
    ...prev,
    warranties: prev.warranties.filter((_, idx) => idx !== i)
  }));
  ...
  const generateSmartPrices = async () => {
    ...
    try {
      ...
      if (data.priceEvaluation) {
        ...
        setPropertyData(prev => ({
          ...prev,
          priceStatus: data.priceEvaluation.status
        }));
      }
  ...
  const togglePlatform = (platformId: string) => {
    setPropertyData(prev => ({
      ...prev,
      selectedPlatforms: prev.selectedPlatforms.includes(platformId) ? prev.selectedPlatforms.filter(id => id !== platformId) : [...prev.selectedPlatforms, platformId]
    }));
  };
  ...
  const handlePublish = async () => {
    ...
    if (!propertyData.ownerName || !propertyData.ownerPhone) {
      toast.error('يرجى ملء معلومات المالك');
      return;
    }
    ...
    if (propertyData.selectedPlatforms.length === 0) {
      toast.error('يرجى اختيار منصة واحدة على الأقل للنشر');
      return;
    }
  ...
  const executePublish = async () => {
    ...
    try {
      ...
      // إرسال حدث للتحديث
      window.dispatchEvent(new CustomEvent('platformOfferPublished', {
        detail: platformOffer
      }));
  ...
  return <ScrollArea className="h-full">
      ...
      <div className="space-y-2 sm:space-y-3 p-2 sm:p-3 pb-24 max-w-full overflow-x-hidden py-[16px] px-[16px]" dir="rtl">
        
        {/* نافذة استرداد المسودة */}
        {showRecoveryDialog && <Card className="border-2 border-[hsl(var(--gold))] bg-gradient-to-br from-amber-50 to-yellow-50 shadow-lg">
        ...
        {showSuccessActions && publishedAd && <Card className="border-2 border-green-500 bg-gradient-to-br from-green-50 to-emerald-50 shadow-lg">
            <CardContent className="py-6">
              ...
              <PublishSuccessActions publishedAd={publishedAd} onRepublish={handleRepublish} onNavigateToOwner={handleNavigateToOwner} brokerInfo={{
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
          ...
          setPropertyData(getDefaultPropertyData());
          localStorage.removeItem(STORAGE_KEY);
          ...
          <CardContent className="space-y-3 pt-3 px-2 sm:px-6 sm:space-y-4 sm:pt-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
              <div>
                ...
                <Input value={propertyData.ownerName} onChange={e => setPropertyData(prev => ({
                ...prev,
                ownerName: e.target.value
              }))} placeholder="اسم المالك" className="border-[#D4AF37] text-xs sm:text-sm h-9 sm:h-10" />
              </div>
              <div>
                <Label className="text-[#01411C] text-xs sm:text-sm">تاريخ الميلاد</Label>
                <Input type="date" value={propertyData.ownerBirthDate} onChange={e => setPropertyData(prev => ({
                ...prev,
                ownerBirthDate: e.target.value
              }))} className="border-[#D4AF37] text-xs sm:text-sm h-9 sm:h-10" />
              </div>
              <div>
                <Label className="text-[#01411C] text-xs sm:text-sm">رقم الهوية *</Label>
                <Input value={propertyData.ownerIdNumber} onChange={e => setPropertyData(prev => ({
                ...prev,
                ownerIdNumber: e.target.value.replace(/[^0-9]/g, '').slice(0, 10)
              }))} placeholder="10 أرقام" className="border-[#D4AF37] text-xs sm:text-sm h-9 sm:h-10" dir="ltr" />
              </div>
              <div>
                <Label className="text-[#01411C] text-xs sm:text-sm">رقم الجوال *</Label>
                <Input value={propertyData.ownerPhone} onChange={e => setPropertyData(prev => ({
                ...prev,
                ownerPhone: e.target.value
              }))} placeholder="05xxxxxxxx" className="border-[#D4AF37] text-xs sm:text-sm h-9 sm:h-10" dir="ltr" />
              </div>
              <div>
                <Label className="text-[#01411C] text-xs sm:text-sm">المدينة</Label>
                <Select value={propertyData.ownerCity} onValueChange={v => setPropertyData(prev => ({
                ...prev,
                ownerCity: v
              }))}>
              ...
              <div>
                ...
                <Input value={propertyData.ownerDistrict} onChange={e => setPropertyData(prev => ({
                ...prev,
                ownerDistrict: e.target.value
              }))} placeholder="اسم الحي" className="border-[#D4AF37] text-xs sm:text-sm h-9 sm:h-10" />
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-3 pt-3 px-2 sm:px-6 sm:space-y-4 sm:pt-4">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3">
              <div>
                ...
                <Input value={propertyData.deedNumber} onChange={e => setPropertyData(prev => ({
                ...prev,
              ...
              <div>
                ...
                <Input type="date" value={propertyData.deedDate} onChange={e => setPropertyData(prev => ({
                ...prev,
              ...
              <div>
                ...
                <Select value={propertyData.deedCity} onValueChange={v => setPropertyData(prev => ({
                ...prev,
                deedCity: v
              }))}>
          ...
          <CardContent className="space-y-3 pt-3 px-2 sm:px-6 sm:space-y-4 sm:pt-4">
            <div className="grid grid-cols-2 gap-2 sm:gap-3">
              <div>
                ...
                <Select value={propertyData.propertyType} onValueChange={v => setPropertyData(prev => ({
                ...prev,
                propertyType: v
              }))}>
                  <SelectTrigger className="border-[#D4AF37] text-xs sm:text-sm h-9 sm:h-10"><SelectValue placeholder="اختر" /></SelectTrigger>
              ...
              <div>
                ...
                <Select value={propertyData.category} onValueChange={v => setPropertyData(prev => ({
                ...prev,
                category: v
              }))}>
                  <SelectTrigger className="border-[#D4AF37] text-xs sm:text-sm h-9 sm:h-10"><SelectValue placeholder="اختر" /></SelectTrigger>
              ...
              <div>
                ...
                <Select value={propertyData.purpose} onValueChange={v => setPropertyData(prev => ({
                ...prev,
                purpose: v
              }))}>
                  <SelectTrigger className="border-[#D4AF37] text-xs sm:text-sm h-9 sm:h-10"><SelectValue placeholder="اختر" /></SelectTrigger>
              ...
              <div>
                ...
                <Input type="number" value={propertyData.area} onChange={e => setPropertyData(prev => ({
                ...prev,
                area: e.target.value
              }))} className="border-[#D4AF37] text-xs sm:text-sm h-9 sm:h-10" />
              </div>
          ...
          <CardContent className="space-y-4 pt-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Select value={propertyData.locationDetails.city} onValueChange={v => setPropertyData(prev => ({
                ...prev,
                locationDetails: {
                  ...prev.locationDetails,
                  city: v
              ...
              <div>
                ...
                <Input value={propertyData.locationDetails.district} onChange={e => setPropertyData(prev => ({
                ...prev,
                locationDetails: {
                  ...prev.locationDetails,
                  district: e.target.value
                }
              }))} className="border-[#D4AF37] text-sm" />
              </div>
              <div>
                <Label className="text-[#01411C] text-sm">الشارع</Label>
                <Input value={propertyData.locationDetails.street} onChange={e => setPropertyData(prev => ({
                ...prev,
                locationDetails: {
                  ...prev.locationDetails,
              ...
              <div>
                ...
                <Input value={propertyData.locationDetails.postalCode} onChange={e => setPropertyData(prev => ({
                ...prev,
                locationDetails: {
                  ...prev.locationDetails,
                  postalCode: e.target.value
                }
              }))} className="border-[#D4AF37] text-sm" dir="ltr" />
              </div>
              <div>
                <Label className="text-[#01411C] text-sm">رقم المبنى</Label>
                <Input value={propertyData.locationDetails.buildingNumber} onChange={e => setPropertyData(prev => ({
                ...prev,
                locationDetails: {
                  ...prev.locationDetails,
              ...
              <div>
                ...
                <Input value={propertyData.locationDetails.additionalNumber} onChange={e => setPropertyData(prev => ({
                ...prev,
                locationDetails: {
                  ...prev.locationDetails,
            ...
            <div>
              ...
              <Input value={propertyData.ownerNationalAddress} onChange={e => setPropertyData(prev => ({
              ...prev,
              ownerNationalAddress: e.target.value
            }))} placeholder="يتم تعبئته تلقائياً من الخريطة" className="border-[#D4AF37] bg-green-50" />
            </div>
          </CardContent>
        </Card>

        {/* ===================== الوسائط ===================== */}
        <PropertyMediaUpload media={propertyData.media} onMediaChange={media => setPropertyData(prev => ({
        ...prev,
        media
      }))} tour3DUrl={propertyData.tour3DUrl} onTour3DChange={url => setPropertyData(prev => ({
        ...prev,
        tour3DUrl: url
      }))} />
          ...
          <CardContent className="pt-4">
            <div className="p-4 bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg border-2 border-purple-300">
              <div className="flex items-center gap-2 flex-wrap">
                {smartPath ? smartPath.split(' / ').map((part, i) => <span key={i} className="flex items-center">
                  ...
                  </span>) : <span className="text-purple-600 text-sm">يتم بناء المسار تلقائياً</span>}
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-2 border-[#D4AF37]">
          ...
          <CardContent className="space-y-4 pt-4">
            <div className="grid grid-cols-3 gap-2">
              <div><Label className="text-[#01411C] text-xs">غرف النوم</Label><Input type="number" value={propertyData.bedrooms} onChange={e => setPropertyData(prev => ({
                ...prev,
                bedrooms: e.target.value
              }))} className="border-[#D4AF37] text-sm h-9" /></div>
              <div><Label className="text-[#01411C] text-xs">دورات المياه</Label><Input type="number" value={propertyData.bathrooms} onChange={e => setPropertyData(prev => ({
                ...prev,
                bathrooms: e.target.value
              }))} className="border-[#D4AF37] text-sm h-9" /></div>
              <div><Label className="text-[#01411C] text-xs">الصالات</Label><Input type="number" value={propertyData.livingRooms} onChange={e => setPropertyData(prev => ({
                ...prev,
                livingRooms: e.target.value
              }))} className="border-[#D4AF37] text-sm h-9" /></div>
              <div><Label className="text-[#01411C] text-xs">الأدوار</Label><Input type="number" value={propertyData.floors} onChange={e => setPropertyData(prev => ({
                ...prev,
                floors: e.target.value
              }))} className="border-[#D4AF37] text-sm h-9" /></div>
              <div><Label className="text-[#01411C] text-xs">عمر العقار</Label><Input type="number" value={propertyData.propertyAge} onChange={e => setPropertyData(prev => ({
                ...prev,
                propertyAge: e.target.value
              }))} className="border-[#D4AF37] text-sm h-9" /></div>
              <div><Label className="text-[#01411C] text-xs">عرض الشارع</Label><Input type="number" value={propertyData.streetWidth} onChange={e => setPropertyData(prev => ({
                ...prev,
            ...
            <div className="grid grid-cols-2 gap-3 mt-3">
              <div>
                ...
                <Select value={propertyData.facade} onValueChange={v => setPropertyData(prev => ({
                ...prev,
                facade: v
              }))}>
              ...
              <div>
                ...
                <Select value={propertyData.furnishing} onValueChange={v => setPropertyData(prev => ({
                ...prev,
                furnishing: v
              }))}>
                  <SelectTrigger className="border-[#D4AF37] text-sm h-9"><SelectValue placeholder="اختر" /></SelectTrigger>
        ...
        {/* ===================== معلومات الإيجار ===================== */}
        {propertyData.purpose === 'للإيجار' && <Card className="border-2 border-orange-400">
            ...
            <CardContent className="space-y-4 pt-4">
              <div className="flex items-center gap-3">
                <Checkbox id="isRented" checked={propertyData.isCurrentlyRented} onCheckedChange={c => setPropertyData(prev => ({
              ...prev,
              isCurrentlyRented: c as boolean
            }))} />
                <Label htmlFor="isRented" className="text-orange-700 font-medium">العقار مؤجر حالياً</Label>
              </div>
              {propertyData.isCurrentlyRented && <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 p-3 bg-orange-50 rounded-lg border border-orange-200">
                  <div>
                    <Label className="text-orange-700 text-sm">مدة العقد (شهور)</Label>
                    <Input type="number" value={propertyData.contractDuration} onChange={e => setPropertyData(prev => ({
                ...prev,
                contractDuration: e.target.value
              }))} className="border-orange-300 text-sm" />
                  </div>
                  <div>
                    <Label className="text-orange-700 text-sm">تاريخ بداية العقد</Label>
                    <Input type="date" value={propertyData.contractStartDate} onChange={e => setPropertyData(prev => ({
                ...prev,
                contractStartDate: e.target.value
                  ...
                  <div>
                    ...
                    <Input type="date" value={propertyData.contractEndDate} onChange={e => setPropertyData(prev => ({
                ...prev,
        ...
        <Card className="border-2 border-[#D4AF37]">
          ...
          <CardContent className="space-y-4 pt-4">
            <div className="flex flex-wrap gap-2">
              {defaultFeatures.map(f => <Badge key={f} variant={propertyData.features.includes(f) ? "default" : "outline"} className={`cursor-pointer ${propertyData.features.includes(f) ? 'bg-[#01411C] text-white' : 'border-[#D4AF37] text-[#01411C]'}`} onClick={() => toggleFeature(f)}>
        ...
        <Card className="border-2 border-[#D4AF37]">
          ...
          <CardContent className="space-y-4 pt-4">
            ...
            {propertyData.warranties.length > 0 && <div className="space-y-2">
                {propertyData.warranties.map((w, i) => <div key={i} className="flex items-center justify-between p-3 bg-green-50 rounded-lg border border-green-200">
        ...
        <Card className="border-2 border-[#D4AF37]">
          ...
          <CardContent className="space-y-4 pt-4">
            ...
            {suggestedPrices.length > 0 && <RadioGroup value={propertyData.priceSource} onValueChange={v => {
            const s = suggestedPrices.find(p => p.source === v);
            if (s) setPropertyData(prev => ({
              ...prev,
              priceSource: v,
              price: s.price.replace(/,/g, '')
            }));
          }}>
                {suggestedPrices.map((s, i) => <div key={i} className="flex items-center p-3 border rounded-lg hover:bg-gray-50">
            ...
            <div>
              ...
              <Input type="number" value={propertyData.price} onChange={e => setPropertyData(prev => ({
              ...prev,
              price: e.target.value
            }))} className="border-[#D4AF37]" />
            </div>
            {priceEvaluation && <div className={`p-3 rounded-lg border-2 ${priceEvaluation.color === 'green' ? 'bg-green-50 border-green-300' : priceEvaluation.color === 'blue' ? 'bg-blue-50 border-blue-300' : 'bg-red-50 border-red-300'}`}>
            ...
            {propertyData.purpose === 'للإيجار' && <div className="grid grid-cols-2 gap-3 p-3 bg-amber-50 rounded-lg border border-amber-200">
                <div><Label className="text-amber-700 text-xs">دفعة واحدة</Label><Input type="number" value={propertyData.paymentPrices.onePayment} onChange={e => setPropertyData(prev => ({
                ...prev,
                paymentPrices: {
                  ...prev.paymentPrices,
                  onePayment: e.target.value
                }
              }))} className="border-amber-300 text-sm h-9" /></div>
                <div><Label className="text-amber-700 text-xs">دفعتين</Label><Input type="number" value={propertyData.paymentPrices.twoPayments} onChange={e => setPropertyData(prev => ({
                ...prev,
                paymentPrices: {
                  ...prev.paymentPrices,
                  twoPayments: e.target.value
                }
              }))} className="border-amber-300 text-sm h-9" /></div>
                <div><Label className="text-amber-700 text-xs">أربع دفعات</Label><Input type="number" value={propertyData.paymentPrices.fourPayments} onChange={e => setPropertyData(prev => ({
                ...prev,
                paymentPrices: {
                  ...prev.paymentPrices,
                  fourPayments: e.target.value
                }
              }))} className="border-amber-300 text-sm h-9" /></div>
                <div><Label className="text-amber-700 text-xs">شهري</Label><Input type="number" value={propertyData.paymentPrices.monthly} onChange={e => setPropertyData(prev => ({
                ...prev,
                paymentPrices: {
                  ...prev.paymentPrices,
        ...
        <Card className="border-2 border-[#D4AF37]">
          ...
          <CardContent className="space-y-4 pt-4">
            <div className="flex flex-wrap gap-2">
              {autoHashtags.map(h => <Badge key={h} variant={propertyData.hashtags.includes(h) ? "default" : "outline"} className={`cursor-pointer ${propertyData.hashtags.includes(h) ? 'bg-indigo-600 text-white' : 'border-indigo-300 text-indigo-600'}`} onClick={() => toggleHashtag(h)}>{h}</Badge>)}
            </div>
            <div className="flex gap-2">
        ...
        <Card className="border-2 border-[#D4AF37]">
          ...
          <CardContent className="space-y-4 pt-4">
            <div className="grid grid-cols-2 gap-3">
              <div><Label className="text-[#01411C] text-sm">رقم الوسيط</Label><Input value={propertyData.brokerPhone} onChange={e => setPropertyData(prev => ({
                ...prev,
                brokerPhone: e.target.value
              }))} className="border-[#D4AF37] text-sm" dir="ltr" /></div>
              <div><Label className="text-[#01411C] text-sm">الترخيص الإعلاني *</Label><Input value={propertyData.adLicense} onChange={e => setPropertyData(prev => ({
                ...prev,
                adLicense: e.target.value
              }))} className="border-[#D4AF37] text-sm" /></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div><Label className="text-[#01411C] text-sm">تاريخ الترخيص</Label><Input type="date" value={propertyData.adLicenseDate} onChange={e => setPropertyData(prev => ({
                ...prev,
                adLicenseDate: e.target.value
              }))} className="border-[#D4AF37] text-sm" /></div>
              <div>
                <Label className="text-[#01411C] text-sm">مدة الترخيص</Label>
                <Select value={propertyData.adLicenseDuration} onValueChange={v => setPropertyData(prev => ({
                ...prev,
                adLicenseDuration: v
              }))}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Select value={propertyData.descriptionLength} onValueChange={v => setPropertyData(prev => ({
                ...prev,
                descriptionLength: v
              }))}>
                  <SelectTrigger className="border-[#D4AF37] text-sm"><SelectValue /></SelectTrigger>
              ...
              <div>
                ...
                <Select value={propertyData.descriptionLanguage} onValueChange={v => setPropertyData(prev => ({
                ...prev,
                descriptionLanguage: v
              }))}>
                  <SelectTrigger className="border-[#D4AF37] text-sm"><SelectValue /></SelectTrigger>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {['احترافي', 'تسويقي', 'فاخر'].map(s => <Button key={s} size="sm" variant={propertyData.descriptionStyle === s ? "default" : "outline"} className={propertyData.descriptionStyle === s ? "bg-[#01411C] text-white" : "border-[#D4AF37]"} onClick={() => setPropertyData(prev => ({
                ...prev,
                descriptionStyle: s
              }))}>{s}</Button>)}
              </div>
            ...
            <AIDescription mode={propertyData.purpose === 'للبيع' ? 'sale' : 'rent'} city={propertyData.locationDetails.city} district={propertyData.locationDetails.district} propertyType={propertyData.propertyType} features={{
            category: propertyData.category,
          ...
          }} price={propertyData.price ? parseInt(propertyData.price) : undefined} currentDescription={propertyData.aiDescription} currentTitle={propertyData.aiTitle} onDescriptionSelect={d => setPropertyData(prev => ({
            ...prev,
            aiDescription: d
          }))} onTitleSelect={t => setPropertyData(prev => ({
            ...prev,
            aiTitle: t
          }))} style={propertyData.descriptionStyle as any} length={propertyData.descriptionLength as any} language={propertyData.descriptionLanguage as any} brokerPhone={propertyData.brokerPhone} adLicense={propertyData.adLicense} />

            <div>
              <Label className="text-[#01411C] text-sm">عنوان الإعلان</Label>
              <Input value={propertyData.aiTitle} onChange={e => setPropertyData(prev => ({
              ...prev,
              aiTitle: e.target.value
            }))} placeholder="اضغط توليد للعنوان الذكي" className="border-[#D4AF37] bg-green-50 font-medium text-sm" />
            </div>
            <div>
              <Label className="text-[#01411C] text-sm">الوصف</Label>
              <Textarea value={propertyData.aiDescription} onChange={e => setPropertyData(prev => ({
              ...prev,
              aiDescription: e.target.value
            }))} placeholder="اضغط توليد للوصف الذكي" className="border-[#D4AF37] min-h-[100px] bg-green-50 text-sm" />
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-2 border-purple-400">
          ...
          <CardContent className="space-y-4 pt-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {connectedPlatforms.map(platform => <motion.div key={platform.id} whileTap={{
              scale: 0.95
            }} onClick={() => platform.status === 'connected' && togglePlatform(platform.id)} className={`p-3 rounded-lg border-2 cursor-pointer transition-all ${propertyData.selectedPlatforms.includes(platform.id) ? 'border-purple-500 bg-purple-50' : platform.status === 'connected' ? 'border-gray-200 hover:border-purple-300' : 'border-gray-100 bg-gray-50 opacity-50 cursor-not-allowed'}`}>
        ...
        <Card className="border-2 border-[#01411C] bg-gradient-to-r from-[#01411C]/5 to-[#D4AF37]/5">
          <CardContent className="p-4">
            ...
            {/* شريط التقدم */}
            {isPublishing && <div className="mb-4">
            ...
            {/* نتائج النشر */}
            {showResults && publishResults.length > 0 && <div className="mb-4 space-y-2">
                <h4 className="font-bold text-[#01411C]">نتائج النشر:</h4>
                {publishResults.map((r, i) => <div key={i} className={`flex items-center justify-between p-3 rounded-lg ${r.status === 'success' ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
                    <div className="flex items-center gap-2">
            ...
            <div className="flex gap-3">
              ...
              <Button onClick={handlePublish} disabled={isPublishing || !propertyData.propertyType || !propertyData.purpose || !propertyData.locationDetails.city || !propertyData.ownerName || !propertyData.ownerPhone || propertyData.selectedPlatforms.length === 0} className="flex-1 bg-[#01411C] hover:bg-[#01411C]/90 text-[#D4AF37] font-bold text-sm py-5">
                {isPublishing ? <><Loader2 className="w-4 h-4 ml-1 animate-spin" />جاري النشر...</> : <><Send className="w-4 h-4 ml-1" />نشر الإعلان</>}
              </Button>
            </div>

            {(!propertyData.propertyType || !propertyData.purpose || !propertyData.locationDetails.city || !propertyData.ownerName || !propertyData.ownerPhone || propertyData.selectedPlatforms.length === 0) && <div className="mt-3 flex items-center gap-2 text-amber-600 text-sm">
      ...
      <AlertDialog open={showDuplicateWarning} onOpenChange={setShowDuplicateWarning}>
        <AlertDialogContent className="max-w-md" dir="rtl">
          ...
          <AlertDialogFooter className="flex-row-reverse gap-2">
            <AlertDialogAction onClick={() => executePublish()} className="bg-amber-600 hover:bg-amber-700 text-white">
              نشر على أي حال
            </AlertDialogAction>
